...
selected=${VOXFORGE_ROOT}/selected
...
local/voxforge_select.sh --dialect $dialects \
  ${VOXFORGE_ROOT}/extracted ${selected} || exit 1
...
local/laps_data_prep.sh || exit 1

#######################################################
# merge databases: Laps + Voxforge
# All Laps database will be used in training step
cat data/laps/wav.scp >> data/local/train_wav.scp
cat data/laps/text >> data/local/train_trans.txt
cat data/laps/utt2spk >> data/local/train.utt2spk
cat data/laps/spk2gender >> data/local/spk2gender

sort data/local/train_wav.scp -o data/local/train_wav.scp
sort data/local/train_trans.txt -o data/local/train_trans.txt
sort data/local/train.utt2spk -o data/local/train.utt2spk
utils/utt2spk_to_spk2utt.pl data/local/train.utt2spk > data/local/train.spk2utt
sort data/local/spk2gender -o data/local/spk2gender
########################################################
