...
BaseFloat DnnGop::ComputeGopNumera(nnet3::DecodableAmNnetSimple &decodable,
  ...
  std::vector<int32> phoneseq(ctx_dep_.ContextWidth());
  if (ctx_dep_.ContextWidth() == 3){
    phoneseq[0] = phone_l;
    phoneseq[1] = phone;
    phoneseq[2] = phone_r;
  } else if(ctx_dep_.ContextWidth() == 2){
      phoneseq[0] = phone_l;
      phoneseq[1] = phone;
  }
...
BaseFloat DnnGop::ComputeGopDenominViterbi(nnet3::DecodableAmNnetSimple &decodable,
                                    int32 phone_l, int32 phone_r) {
    KALDI_ASSERT(ctx_dep_.CentralPosition() == 1);
    std::vector<int32> phoneseq(ctx_dep_.ContextWidth());
    if (ctx_dep_.ContextWidth() == 3){
...
void DnnGop::Compute(const Matrix<BaseFloat> &feats,
  ...
  for (MatrixIndexT i = 0; i < split.size(); i++) {
    ...
    nnet3::DecodableAmNnetSimple split_decodable(decodable_opts, tm_, am_, features,
                                                 NULL, &online_ivectors_feat,
                                                 online_ivector_period, NULL);

    int32 phone, phone_l, phone_r;
    GetContextFromSplit(split, i, phone_l, phone, phone_r);

    bool use_viterbi_numera = true;
    BaseFloat gop_numerator = ComputeGopNumeraViterbi(split_decodable, phone_l, phone, phone_r); //ComputeGopNumera(ali_decodable, phone_l, phone, phone_r, frame_start_idx, split[i].size());
    BaseFloat gop_denominator = ComputeGopDenominViterbi(split_decodable, phone_l, phone_r); //ComputeGopDenomin(ali_decodable, phone_l, phone_r, frame_start_idx, split[i].size());
